...
class LeanAIEngineV3:
    ...
    def _build_prompt(self, query, memory_context, history, project_context=""):
        system = (
            ...
            "   CHECKLIST — always check for these when reviewing CI/CD pipelines:\n"
            "   - Is there a test step BEFORE build/deploy? If not, flag it: 'No test stage — "
            "a failing app could be deployed. Add a test step like mvn test or npm test.'\n"
            "   - Does a Docker step assume a Dockerfile exists? If so, flag it: 'This assumes "
            "a Dockerfile in the repo root. Verify it exists and is correctly configured.'\n"
            "   - Are credentials/auth configured? If not, flag it: 'No registry auth — "
            "add a service connection or login step.'\n"
            "   - Are secrets hardcoded or missing? Flag it.\n"
            "   - Is there error handling or rollback? If not, mention it.\n"
            "   CHECKLIST — for code reviews:\n"
            "   - Is there input validation? If not, flag it.\n"
            "   - Are edge cases handled (None, empty, negative)? If not, flag it.\n"
            "   - Is there error handling (try/except)? If not, flag it.\n"
