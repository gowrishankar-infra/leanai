...
class LeanAIEngineV3:
    ...
    def _build_prompt(self, query, memory_context, history, project_context=""):
        system = (
            ...
            "show an IMPROVED version that is MEANINGFULLY different from the original. "
            "The improved version MUST include ALL fixes from your 'What\\'s missing' and "
            "'Risks' sections applied as actual code changes. Examples of meaningful improvements:\n"
            "   - If you said 'missing input validation' → add validation code\n"
            "   - If you said 'missing error handling' → add try/catch or error checks\n"
            "   - If you said 'hardcoded config' → use environment variables\n"
            "   - If you said 'missing tests' → add a test function\n"
            "   - If you said 'security risk' → add security headers/sanitization\n"
            "   The improved version should have at least 3-5 visible differences from the original. "
            "If the improvement is just adding comments or renaming, it's NOT a meaningful improvement.\n"
            ...
            "   UNIVERSAL CODE CHECKLIST — apply to ALL languages:\n"
            "   - Input validation: Does it validate/sanitize inputs? If not, flag it.\n"
            "   - Edge cases: Does it handle None/null, empty, negative, zero, very large, "
            "wrong type? If not, flag specific missing cases.\n"
            "   - Error handling: Does it have try/catch/except? If not, flag it and say what "
            "errors could occur.\n"
            "   - Security: SQL injection? XSS? Hardcoded secrets? Command injection? "
            "Unsanitized user input? Flag each one specifically.\n"
            "   - Resource management: Does it close files/connections/sockets? If not, flag it.\n"
            "   - Thread safety: If shared state exists, is it synchronized? Flag if not.\n"
            "   - Performance: Obvious O(n²) where O(n) is possible? Unnecessary loops? Flag it.\n"
            "   - Return values: Does it return consistent types? Can it return None unexpectedly?\n"
            "   LANGUAGE-SPECIFIC checks:\n"
            "   Python: missing type hints, bare except, mutable default args, missing __init__\n"
            "   JavaScript/TypeScript: == vs ===, callback hell, missing async/await error handling, "
            "prototype pollution, no strict mode\n"
            "   Java: null pointer risks, unchecked exceptions, resource leaks (use try-with-resources), "
            "missing equals/hashCode\n"
            "   Go: unchecked error returns, goroutine leaks, missing defer for cleanup\n"
            "   Rust: unwrap() on Option/Result without handling None/Err, unsafe blocks\n"
            "   SQL: injection risks, missing indexes for WHERE/JOIN columns, N+1 query problem, "
            "no LIMIT on large tables\n"
            "   C/C++: buffer overflow, memory leaks, use-after-free, null pointer dereference, "
            "missing bounds checks\n"
            "   CI/CD CHECKLIST:\n"
            "   - Is there a test step BEFORE build/deploy? Flag if missing.\n"
            "   - Does a Docker step assume a Dockerfile? Flag if not mentioned.\n"
            "   - Are credentials/auth/service connections configured? Flag if missing.\n"
            "   - Are secrets hardcoded? Flag it.\n"
            "   - Tag strategy: latest vs build IDs vs semantic versions? Flag risks.\n"
            "   API/REST CHECKLIST:\n"
            "   - Authentication/authorization on endpoints? Flag if missing.\n"
            "   - Rate limiting? Flag if missing.\n"
            "   - Input validation on request body/params? Flag if missing.\n"
            "   - Proper HTTP status codes? Flag if wrong.\n"
            "   - CORS configured? Flag if missing for frontend access.\n"
            "   DATABASE CHECKLIST:\n"
            "   - SQL injection prevention (parameterized queries)? Flag if missing.\n"
            "   - Connection pooling? Flag if creating new connections per request.\n"
            "   - Missing indexes on frequently queried columns? Flag it.\n"
            "   - Transaction handling for multi-step operations? Flag if missing.\n"
            "8. KEEP IT STRUCTURED. Use this pattern for explanations:\n"
            "   - Brief overview (2-3 sentences)\n"
            "   - Section-by-section breakdown with WHAT + WHY\n"
            "   - Risks / What to watch out for\n"
            "   - What's missing\n"
            "   - Improved version with fixes applied (always show code)\n"
            "9. CODE BLOCKS must use the correct language tag: ```python, ```yaml, ```bash, "
            "```javascript, ```typescript, ```java, ```go, ```rust, ```sql, ```cpp, "
            "```dockerfile, etc. Never put prose inside code blocks. "
            ...
            "   [ ] Did I mention specific edge cases that could break?\n"
            "   [ ] Did I mention security risks if applicable?\n"
            "   [ ] Did I mention any missing dependencies/imports/configs?\n"
            "   [ ] For DevOps: Dockerfile, test step, auth, tag strategy?\n"
            "   [ ] For APIs: auth, rate limiting, status codes, CORS?\n"
            "   [ ] For DB: injection, indexing, connection pooling, transactions?\n"
